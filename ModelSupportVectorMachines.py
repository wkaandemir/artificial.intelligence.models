...
def run():
    ...
    def save_results(result):
        ...
        output_path = os.path.join(output_folder, 'ModelSupportVectorMachines.txt')
    ...
    param_grid = SupportVectorMachines_hyperparameters

    # Create an SVM model
    svm_model = SVC(probability=True, random_state=42)

    # Use the model and parameter combinations for Grid Search
    grid_search = GridSearchCV(estimator=svm_model, param_grid=param_grid, scoring='accuracy', cv=3, verbose=2)
    ...
    # Save the model
    model_path = os.path.join(models_folder, 'ModelSupportVectorMachines.pkl')
    ...
    brier_score = brier_score_loss(y_test, best_svm_model.predict_proba(X_test)[:, 1])
